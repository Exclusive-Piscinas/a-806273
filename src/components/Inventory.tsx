...
import { Package, Plus, ArrowUp, ArrowDown, ChevronRight, X, Check, Save, FileUp, FileDown, BarChart2, Search, Filter, Trash2 } from 'lucide-react';
...
import { exportInventoryToCSV, importInventoryFromCSV, exportInventoryToPDF, downloadInventoryTemplate, InventoryItem } from './inventory/ImportExportFunctions';
...
const initialTransactionHistory = [{
  ...
  date: '2023-08-18',
  user: 'Jean Dupont',
  notes: 'Application parcelle est'
}, {
  id: 3,
  itemId: 4,
  type: 'in',
  ...
  date: '2023-08-15',
  user: 'Jean Dupont',
  notes: 'Application parcelle sud'
}, {
  id: 5,
  itemId: 1,
  type: 'in',
...
const Inventory: React.FC<InventoryProps> = ({
  ...
  const generateAlerts = () => {
    return inventoryData.filter(item => item.quantity <= item.minQuantity).map(item => ({
      id: item.id,
      name: item.name,
  ...
  const filteredItems = inventoryData.filter(item => {
    ...
    if (sortBy === 'name') {
      ...
      return sortOrder === 'asc' ? new Date(a.lastUpdated).getTime() - new Date(b.lastUpdated).getTime() : new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime();
    }
    return 0;
  });
  ...
  const handleDeleteItem = () => {
    ...
    setCategoryStats(categoryStats.map(stat => stat.name === itemToDeleteObj.category ? {
      ...stat,
      value: Math.max(0, stat.value - itemToDeleteObj.quantity)
    } : stat));
  ...
  const handleDeleteTransaction = () => {
    ...
    handleUpdateItem(selectedItem.id, 'quantity', Math.max(0, selectedItem.quantity + quantityChange));
  ...
  const handleAddItem = () => {
    ...
    if (existingCategoryStat) {
      setCategoryStats(categoryStats.map(stat => stat.name === newItem.category ? {
        ...stat,
        value: stat.value + Number(newItem.quantity)
      } : stat));
    } else {
  ...
  const handleUpdateItem = (id: number, field: string, value: any) => {
    ...
    if (field === 'quantity') {
      ...
      if (item) {
        ...
        setCategoryStats(categoryStats.map(stat => stat.name === item.category ? {
          ...stat,
          value: stat.value + diff
        } : stat));
      }
    }
  };
  ...
  const handleSubmitTransaction = () => {
    ...
    const updatedQuantity = showTransactionForm === 'in' ? selectedItem.quantity + newTransaction.quantity : Math.max(0, selectedItem.quantity - newTransaction.quantity);
  ...
  const itemTransactions = selectedItem ? transactionHistory.filter(t => t.itemId === selectedItem.id).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) : [];
  ...
  const tableData = filteredItems.map(item => ({
    ...
    status: item.quantity <= item.minQuantity ? item.quantity < item.minQuantity * 0.5 ? 'critical' : 'warning' : 'normal'
  }));
  ...
  return <div className="animate-enter">
      <header className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant={view === 'list' ? 'default' : 'outline'} onClick={() => setView('list')} className="px-4 py-2">LISTA</Button>
          <Button variant={view === 'stats' ? 'default' : 'outline'} onClick={() => setView('stats')} className="px-4 py-2">
      ...
      <InventoryAlerts alerts={alerts} onQuantityChange={handleUpdateItem} />

      {view === 'list' ? selectedItem ? <div className="border rounded-xl overflow-hidden">
            <div className="bg-agri-primary text-white p-4 flex justify-between items-center">
              <div className="flex items-center">
                ...
                <EditableField value={selectedItem.name} onSave={value => handleUpdateItem(selectedItem.id, 'name', value)} className="text-xl font-semibold" />
              </div>
              <div className="flex flex-wrap gap-2">
                <Button onClick={() => handleAddTransaction('in')} variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-none">
                  <ArrowDown className="mr-1.5 h-4 w-4" />
                  <span className="hidden sm:inline">Entrée</span>
                </Button>
                <Button onClick={() => handleAddTransaction('out')} variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-none">
            ...
            <div className="p-6">
                ...
                <div className="bg-white rounded-lg border p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <EditableField value={selectedItem.category} onSave={value => handleUpdateItem(selectedItem.id, 'category', value)} />
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-muted-foreground">Quantité:</span>
                      <div className="flex items-center">
                        <EditableField value={selectedItem.quantity} type="number" onSave={value => handleUpdateItem(selectedItem.id, 'quantity', Number(value))} className="font-medium" />
                        <EditableField value={selectedItem.unit} onSave={value => handleUpdateItem(selectedItem.id, 'unit', value)} className="ml-1" />
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-muted-foreground">Seuil minimal:</span>
                      <div className="flex items-center">
                        <EditableField value={selectedItem.minQuantity} type="number" onSave={value => handleUpdateItem(selectedItem.id, 'minQuantity', Number(value))} />
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center">
                        <EditableField value={selectedItem.price} type="number" onSave={value => handleUpdateItem(selectedItem.id, 'price', Number(value))} />
                        <span className="ml-1">€/{selectedItem.unit}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <EditableField value={selectedItem.location} onSave={value => handleUpdateItem(selectedItem.id, 'location', value)} />
                    </div>
                    <div className="flex justify-between items-center">
              ...
              {showTransactionForm && <div className="mb-6 p-4 border rounded-lg bg-muted/10">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-medium">
                      {showTransactionForm === 'in' ? 'Nouvelle entrée' : 'Nouvelle sortie'}
                    </h3>
                    <Button variant="ghost" onClick={() => setShowTransactionForm(null)} size="sm">
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        <Input id="quantity" type="number" value={newTransaction.quantity} onChange={e => setNewTransaction({
                  ...newTransaction,
                ...
                date: e.target.value
                    ...
                    <div>
                      ...
                      <Input id="notes" value={newTransaction.notes} onChange={e => setNewTransaction({
                ...newTransaction,
                notes: e.target.value
              })} placeholder="Commentaire..." className="mt-1" />
                    </div>
                  </div>
                  <div className="flex justify-end mt-4">
                    <Button variant="outline" onClick={() => setShowTransactionForm(null)} className="mr-2">
                      Annuler
                    </Button>
              ...
              <div className="mt-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {itemTransactions.map(transaction => <tr key={transaction.id} className="border-t">
                          <td className="px-4 py-3">{new Date(transaction.date).toLocaleDateString()}</td>
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${transaction.type === 'in' ? 'bg-agri-success/10 text-agri-success' : 'bg-agri-warning/10 text-agri-warning'}`}>
                              {transaction.type === 'in' ? <>
                          ...
                          <td className="px-4 py-3">
                            <EditableField value={transaction.notes} onSave={value => {
                      ...
                      if (index !== -1) {
                        ...
                        setTransactionHistory(updatedTransactions);
                          ...
                          <td className="px-4 py-3">
                            <button onClick={() => confirmDeleteTransaction(transaction.id)} className="p-1.5 hover:bg-agri-danger/10 text-agri-danger rounded" title="Supprimer la transaction">
                              <Trash2 className="h-4 w-4" />
                            </button>
                          </td>
                        </tr>)}
                      {itemTransactions.length === 0 && <tr>
            ...
            <div className="mb-6">
              <InventoryFilters searchTerm={searchTerm} setSearchTerm={setSearchTerm} categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter} categories={categories} sortBy={sortBy} setSortBy={setSortBy} sortOrder={sortOrder} setSortOrder={setSortOrder as (order: 'asc' | 'desc') => void} />
            </div>
            
            <EditableTable data={tableData} columns={inventoryColumns} onUpdate={handleTableUpdate} onDelete={rowIndex => confirmDeleteItem(filteredItems[rowIndex].id)} actions={[{
        icon: <ChevronRight className="h-4 w-4" />,
        label: "Voir détails",
        onClick: rowIndex => setSelectedItem(filteredItems[rowIndex])
      }]} className="mb-6" sortable={true} />
            
            {showAddForm && <div className="border rounded-xl p-6 bg-muted/5 animate-enter">
              ...
              ...newItem,
              name: e.target.value
            })} className="mt-1" placeholder="Ex: Semences de blé" />
                  </div>
                  <div>
            ...
            })} className="mt-1" list="categories-list" placeholder="Ex: Semences" />
                    <datalist id="categories-list">
                      {categories.filter(cat => cat !== 'all').map(category => <option key={category} value={category} />)}
                    </datalist>
                  </div>
                  <div>
                    <Label htmlFor="quantity">Quantité initiale*</Label>
                    <div className="flex mt-1">
                      <Input id="quantity" type="number" value={newItem.quantity} onChange={e => setNewItem({
                ...newItem,
                quantity: Number(e.target.value)
              })} min={0} />
                      <Input className="w-24 ml-2" placeholder="Unité" value={newItem.unit} onChange={e => setNewItem({
                ...newItem,
                unit: e.target.value
              })} />
                    </div>
                  ...
                  <div>
                    ...
                    <Input id="minQuantity" type="number" value={newItem.minQuantity} onChange={e => setNewItem({
              ...newItem,
              minQuantity: Number(e.target.value)
                  ...
                  <div>
                    ...
                    <Input id="price" type="number" value={newItem.price} onChange={e => setNewItem({
              ...newItem,
              price: Number(e.target.value)
            })} className="mt-1" min={0} step="0.01" />
                  </div>
                  <div>
                    <Label htmlFor="location">Emplacement</Label>
                    <Input id="location" value={newItem.location} onChange={e => setNewItem({
              ...newItem,
              location: e.target.value
            })} className="mt-1" placeholder="Ex: Hangar principal" />
                  </div>
              ...
              ...newItem,
              notes: e.target.value
            })} className="mt-1" placeholder="Informations complémentaires sur l'article..." />
                  </div>
                </div>
          ...
          </div> : <InventoryStats inventoryData={inventoryData} categoryStats={categoryStats} />}

      <ConfirmDialog open={deleteConfirmOpen} title="Supprimer l'article" description="Êtes-vous sûr de vouloir supprimer cet article ? Cette action est irréversible." confirmText="Supprimer" cancelText="Annuler" onConfirm={handleDeleteItem} onOpenChange={() => setDeleteConfirmOpen(false)} />

      <ConfirmDialog open={transactionDeleteConfirmOpen} title="Supprimer la transaction" description="Êtes-vous sûr de vouloir supprimer cette transaction ? Le stock sera ajusté en conséquence." confirmText="Supprimer" cancelText="Annuler" onConfirm={handleDeleteTransaction} onOpenChange={() => setTransactionDeleteConfirmOpen(false)} />
    </div>;
};
